...
namespace ET
{
    ...
    {
        public const int CheckInteral = 1000;
        ...
        public static void Check(this SessionIdleCheckerComponent self)
        {
            ...
            if (timeNow - session.LastRecvTime < ConstValue.SessionTimeoutTime &&
                timeNow - session.LastSendTime < ConstValue.SessionTimeoutTime)
            {
                return;
            }
            ...
            EventSystem.Instance.Publish(clientScene,
                new ET.EventType.OnDisconnect() { error = ErrorCore.ERR_SessionSendOrRecvTimeout });
#endif
        }
    }
}