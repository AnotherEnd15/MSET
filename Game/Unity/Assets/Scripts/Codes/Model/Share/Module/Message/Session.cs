...
namespace ET
{
    ...
    public static class SessionSystem
    {
        ...
        public class SessionAwakeSystem: AwakeSystem<Session, int>
        {
            protected override void Awake(Session self, int serviceId)
            {
                ...
                Log.GetLogger().Debug($"session create: zone: {self.DomainZone()} id: {self.Id} {timeNow} ");
                //self.AddComponent<SessionAcceptTimeoutComponent>();
            }
        }
        ...
        public class SessionDestroySystem: DestroySystem<Session>
        {
            protected override void Destroy(Session self)
            {
                ...
                Log.GetLogger().Debug($"session dispose: {self.RemoteAddress} id: {self.Id} ErrorCode: {self.Error}, please see ErrorCode.cs! {TimeHelper.ClientNow()}");
    ...
    public sealed class Session: Entity, IAwake<int>, IDestroy
    {
        ...
        public Dictionary<int, RpcInfo> requestCallbacks = new Dictionary<int, RpcInfo>();
