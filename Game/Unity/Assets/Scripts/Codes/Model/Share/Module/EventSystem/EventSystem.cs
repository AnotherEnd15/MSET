    ...
    public class EventSystem: Singleton<EventSystem>, ISingletonUpdate, ISingletonLateUpdate
    {
            ...
            foreach (IDeserializeSystem deserializeSystem in iDeserializeSystems)
            {
                ...
                catch (Exception e)
                {
                    Log.GetLogger().Error("{TypeName} {Exception}",
                        deserializeSystem.GetType().Name, e);
                }
            }
        }
        ...
        public void AddComponent(Entity entity, Entity component)
        {
            ...
            foreach (IAddComponentSystem addComponentSystem in iAddSystem)
            {
                ...
                {
                    Log.GetLogger().Error("{TypeName} {Exception}",
                        addComponentSystem.GetType().Name, e);
                }
            }
        }
        ...
        public void LateUpdate()
        {
            ...
            while (count-- > 0)
            {
                ...
                foreach (ILateUpdateSystem iLateUpdateSystem in iLateUpdateSystems)
                {
                    ...
                    {
                        Log.GetLogger().Error("{TypeName} {Exception}",
                            iLateUpdateSystem.GetType().Name, e);
                    }
                }
            }
        }

        public async ETTask PublishAsync<E,T>(E entity, T a) where T : struct where E : Entity
        {
            ...
            foreach (EventInfo eventInfo in iEvents)
            {
                ...
                {
                    Log.GetLogger().Error($"event error: {eventInfo.IEvent.GetType().Name}");
                    continue;
                }
        ...
        public void Publish<E,T>(E entity, T a) where T : struct where E: Entity
        {
            List<EventInfo> iEvents;
            if (!this.allEvents.TryGetValue(typeof (T), out iEvents))
            {
            ...
            foreach (EventInfo eventInfo in iEvents)
            {
                if (!(eventInfo.IEvent is AEvent<E,T> aEvent))
                {
                    Log.GetLogger().Error($"event type error: {eventInfo.IEvent.GetType().Name}");
                    continue;
                }
