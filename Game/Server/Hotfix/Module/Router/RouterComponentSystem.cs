...
namespace ET.Server
{
    ...
    public static class RouterComponentSystem
    {
            ...
            switch (flag)
            {
                case KcpProtocalType.RouterReconnectSYN:
                {
                    ...
                    if (!self.OuterNodes.TryGetValue(outerConn, out routerNode))
                    {
                        ...
                        if (routerNode == null)
                        {
                            Log.GetLogger().Information($"router create reconnect: {self.IPEndPoint} {realAddress} {connectId} {outerConn} {innerConn}");
                    ...
                    {
                        Log.GetLogger().Warning($"kcp router router reconnect connectId diff1: {routerNode.SyncIpEndPoint} {(IPEndPoint) self.IPEndPoint}");
                        break;
                    }
                    ...
                    {
                        Log.GetLogger().Warning($"kcp router router reconnect inner conn diff1: {routerNode.SyncIpEndPoint} {(IPEndPoint) self.IPEndPoint}");
                        break;
                    }
                    
                    if (routerNode.OuterConn != outerConn)
                    {
                        Log.GetLogger().Warning($"kcp router router reconnect outer conn diff1: {routerNode.SyncIpEndPoint} {(IPEndPoint) self.IPEndPoint}");
                        break;
                    }
                    ...
                    {
                        Log.GetLogger().Warning($"kcp router syn ip is diff1: {routerNode.SyncIpEndPoint} {(IPEndPoint) self.IPEndPoint}");
                        break;
                    }
                    ...
                    {
                        Log.GetLogger().Warning($"router sync error2: {routerNode.OuterConn} {routerNode.InnerAddress} {outerConn} {realAddress}");
                        break;
                    }
                ...
                case KcpProtocalType.RouterSYN:
                {
                    ...
                    if (routerNode == null)
                    {
                        ...
                        routerNode = self.New(realAddress, connectId, outerConn, innerConn, self.CloneAddress());
                        Log.GetLogger().Information($"router create: {realAddress} {connectId} {outerConn} {innerConn} {routerNode.SyncIpEndPoint}");
                    ...
                    if (!Equals(routerNode.SyncIpEndPoint, self.IPEndPoint))
                    {
                        Log.GetLogger().Warning($"kcp router syn ip is diff1: {routerNode.SyncIpEndPoint} {self.IPEndPoint}");
                        break;
                    }
                ...
                case KcpProtocalType.SYN:
                {
                    ...
                    {
                        Log.GetLogger().Warning($"kcp router syn not found outer nodes: {outerConn} {innerConn}");
                        break;
                    }
                    ...
                    {
                        Log.GetLogger().Warning($"kcp router syn ip is diff3: {kcpRouter.SyncIpEndPoint.Address} {ipEndPoint.Address}");
                        break;
                    }
                    ...
                    Log.GetLogger().Information($"kcp router syn: {outerConn} {innerConn} {kcpRouter.InnerIpEndPoint} {kcpRouter.OuterIpEndPoint}");
                ...
                case KcpProtocalType.FIN: // 断开
                {
                    ...
                    if (!self.OuterNodes.TryGetValue(outerConn, out RouterNode kcpRouter))
                    {
                        Log.GetLogger().Warning($"kcp router outer fin not found outer nodes: {outerConn} {innerConn}");
                        break;
                    }
                    ...
                    {
                        Log.GetLogger().Warning($"router node innerConn error: {innerConn} {outerConn} {kcpRouter.Status}");
                        break;
                    }

                    kcpRouter.LastRecvOuterTime = timeNow;
                    Log.GetLogger().Information($"kcp router outer fin: {outerConn} {innerConn} {kcpRouter.InnerIpEndPoint}");
                ...
                case KcpProtocalType.MSG:
                {
                    ...
                    if (!self.OuterNodes.TryGetValue(outerConn, out RouterNode kcpRouter))
                    {
                        Log.GetLogger().Warning($"kcp router msg not found outer nodes: {outerConn} {innerConn}");
                        break;
                    }

                    if (kcpRouter.Status != RouterStatus.Msg)
                    {
                        Log.GetLogger().Warning($"router node status error: {innerConn} {outerConn} {kcpRouter.Status}");
                        break;
                    }
        ...
        private static void RecvInnerHandler(this RouterComponent self, int messageLength, long timeNow)
        {
            ...
            switch (flag)
            {
                case KcpProtocalType.RouterReconnectACK:
                {
                    ...
                    {
                        Log.GetLogger().Warning($"router node error: {innerConn} {connectId}");
                        break;
                    }
                    ...
                    Log.GetLogger().Information($"kcp router RouterAck: {outerConn} {innerConn} {kcpRouterNode.SyncIpEndPoint}");
                ...
                case KcpProtocalType.ACK:
                {
                    ...
                    {
                        Log.GetLogger().Warning($"kcp router ack not found outer nodes: {outerConn} {innerConn}");
                        break;
                    }
                    ...
                    kcpRouterNode.LastRecvInnerTime = timeNow;
                    // 转发出去
                    Log.GetLogger().Information($"kcp router ack: {outerConn} {innerConn} {kcpRouterNode.OuterIpEndPoint}");
                ...
                case KcpProtocalType.FIN: // 断开
                {
                    ...
                    if (!self.OuterNodes.TryGetValue(outerConn, out RouterNode kcpRouterNode))
                    {
                        Log.GetLogger().Warning($"kcp router inner fin not found outer nodes: {outerConn} {innerConn}");
                        break;
                    }
                    ...
                    {
                        Log.GetLogger().Warning($"router node innerConn error: {innerConn} {outerConn} {kcpRouterNode.Status}");
                        break;
                    }
                    ...
                    kcpRouterNode.LastRecvInnerTime = timeNow;
                    Log.GetLogger().Information($"kcp router inner fin: {outerConn} {innerConn} {kcpRouterNode.OuterIpEndPoint}");
                    self.OuterSocket.SendTo(self.Cache, 0, messageLength, SocketFlags.None, kcpRouterNode.OuterIpEndPoint);
                ...
                case KcpProtocalType.MSG:
                {
                    ...
                    if (!self.OuterNodes.TryGetValue(outerConn, out RouterNode kcpRouterNode))
                    {
                        Log.GetLogger().Warning($"kcp router inner msg not found outer nodes: {outerConn} {innerConn}");
                        break;
                    }
        ...
        private static RouterNode New(this RouterComponent self, string innerAddress, uint connectId, uint outerConn, uint innerConn, IPEndPoint syncEndPoint)
        {
            ...
            Log.GetLogger().Information($"router new: outerConn: {outerConn} innerConn: {innerConn} {syncEndPoint}");
        ...
        public static void OnError(this RouterComponent self, long id, int error)
        {
            ...
            Log.GetLogger().Information($"router node remove: {routerNode.OuterConn} {routerNode.InnerConn} {error}");
            self.Remove(id);
        }
        ...
        private static void Remove(this RouterComponent self, long id)
        {
            ...
            Log.GetLogger().Information($"router remove: {routerNode.Id} outerConn: {routerNode.OuterConn} innerConn: {routerNode.InnerConn}");
