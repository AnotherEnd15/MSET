...
namespace ET.Server
{
    ...
    {
        [ObjectSystem]
        public class AwakeSystem: AwakeSystem<NetServerComponent, string>
        {
            protected override void Awake(NetServerComponent self, string address)
            {
                Log.GetLogger().Information($"Socket 地址开始监听: {address}");
                self.ServiceId = NetServices.Instance.AddService(new KService(IPEndPoint.Parse(address), ServiceType.Outer));
