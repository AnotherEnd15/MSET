...
namespace ET.Server
{
    ...
    public static class ActorMessageSenderComponentSystem
    {
        ...
        private static void Run(ActorMessageSender self, IActorResponse response)
        {
            ...
            {
                self.Tcs.SetException(new Exception($"Rpc error: request, 注意Actor消息超时，请注意查看是否死锁或者没有reply: actorId: {self.ActorId} {self.Request.GetType()}, response: {response.GetType()}"));
                return;
            }

            if (self.NeedException && ErrorCore.IsRpcNeedThrowException(response.Error))
            {
                self.Tcs.SetException(new Exception($"Rpc error: actorId: {self.ActorId} request: {self.Request.GetType()}, response: {response.GetType()}"));
                return;
            }
        ...
        )
        {
            ...
            if (costTime > 3000)
            {
                Log.GetLogger().Warning($"actor rpc time > 1000: {costTime} {iActorRequest.GetType()}");
            }
